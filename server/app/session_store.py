...
class SessionStore:
    ...
    async def put(self, record: SessionRecord) -> SessionRecord:
        async with self._lock:
            ...
            existing = self._sessions.get(record.channel_name)
            if existing is not None and existing.agent_id is not None and record.agent_id is None:
                record.agent_id = existing.agent_id
                record.agent_state = existing.agent_state
